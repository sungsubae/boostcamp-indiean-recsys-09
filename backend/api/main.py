...
if config.env == "dev":
    ...
    fake_user_history = pd.DataFrame(
        columns=["userid", "appid", "playtime_forever", "rtime_last_played"]
    )
...
async def get_user_history(userid: str):
    ...
    fake_user_history = pd.concat(
        [fake_user_history, new_user_history], ignore_index=True
    )
